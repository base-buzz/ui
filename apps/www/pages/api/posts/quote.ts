...
export default async function handler(
  ...
  try {
    ...
    // Add the quote tweet to the top of the posts list
    posts.unshift(newQuoteTweet);

    // Create notification if the quote tweet is from a different user
    ...
    if (!success) {
      return res.status(500).json({ error: "Failed to create quote tweet" });
    }
